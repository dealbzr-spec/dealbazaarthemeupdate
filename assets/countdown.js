    ...
    const observer = new MutationObserver((mutationsList) => {
      mutationsList.forEach(mutation => {
        mutation.addedNodes && mutation.addedNodes.forEach(node => {
          if (node.nodeType === 1) {
            const hasCountdown = node.classList && node.classList.contains('card__countdown');
            const hasCountdownChildren = node.getElementsByClassName && node.getElementsByClassName('card__countdown').length > 0;
            if (hasCountdown || hasCountdownChildren) {
              if (node.matches && node.matches('[data-countdown-id]')) {
                initCountDownForElement(node);
              }
              const foundCountdowns = node.querySelectorAll && node.querySelectorAll('[data-countdown-id]');
